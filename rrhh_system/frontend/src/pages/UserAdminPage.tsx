...
export const UserAdminPage: React.FC = () => {
  ...
  const handleRoleUpdate = async () => {
    ...
    try {
        const response = await axios.patch(`http://127.0.0.1:8000/api/users/${editingUser.id}/`, { role: selectedRole }, {
            headers: { 'Authorization': `Token ${token}` }
        });
        setUsers(users.map(user => user.id === editingUser.id ? response.data : user));
        setIsEditModalOpen(false);
    } catch (err: any) {
        setError(`Error al actualizar: ${err.response?.data?.error || 'Error desconocido'}`);
    }
  };
  ...
  return (
    <div>
                    ...
                    {users.map((user) => (
                        <tr key={user.id}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <button onClick={() => openEditModal(user)} className="text-indigo-600 hover:text-indigo-900 mr-4">Editar Rol</button>
      ...
      <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)}>
        {editingUser && (
            <div>
                <h2 className="text-2xl font-bold mb-6">Editar Rol de {editingUser.username}</h2>
                <div className="mb-4">
                    <label htmlFor="role-select" className="block text-sm font-medium text-gray-700 mb-2">Nuevo Rol</label>
                    <select
                        id="role-select"
                        value={selectedRole}
                ...
                <div className="flex justify-end mt-8">
                    ...
                    <button onClick={handleRoleUpdate} className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 font-semibold">Guardar Cambios</button>
                </div>
            </div>
        )}
      </Modal>
