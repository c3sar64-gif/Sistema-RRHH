...
export const HorasExtrasPage: React.FC = () => {
    ...
    const [formState, setFormState] = useState<HoraExtraFormState>(initialFormState);
    ...
    // Note: Porteria access is not mentioned for Horas Extras, assuming restricted unless specified.
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const events = fetchedData.map((p: HoraExtra) => {
                // Ensure valid date formatting
                const start = moment(`${p.fecha_solicitud}T${p.hora_inicio}`, "YYYY-MM-DDTHH:mm:ss").toDate();
                const end = moment(`${p.fecha_solicitud}T${p.hora_fin}`, "YYYY-MM-DDTHH:mm:ss").toDate();
                return {
                    id: p.id,
                    title: `${p.empleado_info.nombres} ${p.empleado_info.apellido_paterno} - ${p.tipo_hora_extra}`,
                    start: start,
                    end: end,
                    resource: p,
                };
            ...
            // Conditional fetch for Admin/HR/Jefe data needed for dropdowns
            // Jefes need to see employees of their depts.
            // Admin/HR see all.

            // We'll fetch all if Admin/HR. Jefes might leverage the same endpoints if they return filtered data?
            // The endpoints api/empleados/ currently return ALL if Admin/HR, or filtered if Jefes/Staff.
    ...
    useEffect(() => {
        if (selectedJefe) {
            ...
            // For Admin/HR, show all if no Jefe selected? Or clear?
            // If Admin, and no Jefe selected, maybe we shouldn't filter depts by jefe?
            // Existing logic in PermisosPage implies selecting a Jefe is the primary way to filter depts for creation.
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!isAdminOrHR && !isJefeDepto) {
            ...
            // Admin/HR/Jefes assigning to others
            if (!formState.empleado) {
                // Fallback: if they are creating for self?
                // The form logic forces selection from dropdowns for Admin/HR/Jefes.
                // If Jefe wants to create for self, they must select themselves in dropdown (if they are in the list).
        ...
        try {
            ...
            setFormErrors({ general: `Error al registrar: ${JSON.stringify(err.response?.data)}` });
        }
    };
    ...
    // Helper to check if user can approve (Admin, HR, or the assigned approver)
    const canApprove = (item: HoraExtra) => {
        if (isAdminOrHR) return true;
        if (user?.empleado_id === item.aprobador_asignado) return true;
        return false;
    };

    // Helper to check if user can create/view for others
    const canManageOthers = isAdminOrHR || isJefeDepto;
    ...
    return (
        <div>
                ...
                {viewEvent && (
                    <div className="max-w-3xl mx-auto font-sans text-gray-900 border-2 border-gray-800 p-8">
                        ...
                        <div className="text-center border-b-2 border-gray-800 pb-4 mb-6">
                            <h1 className="text-2xl font-bold uppercase">Papeleta de Hora Extra</h1>
                        ...
                        <div className="grid grid-cols-2 gap-x-8 gap-y-4 text-sm mb-12">
                            <div className="col-span-2 flex justify-between items-center mb-2">
                                <div>
                                    <span className="font-bold">N° de Solicitud:</span>
                            ...
                            <div>
                                <span className="font-bold block text-gray-600 text-xs uppercase tracking-wider">Tipo</span>
                                <span className="text-base capitalize">{viewEvent.tipo_hora_extra.replace('_', ' ')}</span>
                            </div>
                            ...
                            <div>
                                <span className="font-bold block text-gray-600 text-xs uppercase tracking-wider">Hora Inicio</span>
                                <span className="text-base">{viewEvent.hora_inicio.substring(0, 5)}</span>
                            </div>

                            <div>
                                <span className="font-bold block text-gray-600 text-xs uppercase tracking-wider">Hora Fin</span>
                                <span className="text-base">{viewEvent.hora_fin.substring(0, 5)}</span>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-16 mt-20 pt-10">
                            <div className="text-center">
                                ...
                                <p className="font-bold">Firma Aprobador</p>
                        ...
                        <div className="mt-12 text-center text-xs text-gray-400">
                            <p>Este documento es constancia de la autorización de horas extras.</p>
            ...
            <div className="flex justify-between items-center mb-6 no-print">
                ...
                {/* Regular employees and managers can register */}
            ...
            {/* Modal de Detalle (Estilo Google Calendar Popover) */}
            <Modal isOpen={!!viewEvent} onClose={() => setViewEvent(null)}>
                {viewEvent && (
                    <div className="space-y-4 no-print">
                        <div className="flex justify-between items-start">
                            <h2 className="text-xl font-bold text-gray-800">{viewEvent.tipo_hora_extra === 'compensacion' ? 'Compensación de Hrs Extra' : 'Horas Extras'}</h2>
                        ...
                        <div className="border-t border-gray-100 pt-3">
                            <div className="grid grid-cols-1 gap-3 text-sm text-gray-600">
                                ...
                                <div className="flex items-center">
                                    ...
                                    <span>{viewEvent.departamento_nombre || '-'}</span>
                                </div>
                                <div className="flex items-center">
                                    <span className="font-semibold w-24">Horario:</span>
                                    <span>{viewEvent.hora_inicio.substring(0, 5)} - {viewEvent.hora_fin.substring(0, 5)}</span>
                                </div>
                        ...
                        <div className="flex justify-end gap-2 pt-4 border-t border-gray-100 mt-4">
                            ...
                            {canApprove(viewEvent) && viewEvent.estado !== 'aprobado' && (
                                <button
                            ...
                            {canApprove(viewEvent) && viewEvent.estado !== 'rechazado' && viewEvent.estado !== 'anulado' && (
                                <button
                            ...
                            {/* Pendiente option mostly for Admin to reset */}
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
                <div className="no-print">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <h2 className="text-2xl font-bold text-gray-800">Registrar Horas Extras</h2>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="time" name="hora_inicio" value={formState.hora_inicio} onChange={handleInputChange} className={inputStyles} />
                            </div>
                            <div>
                                <label>Hora Fin</label>
                                <input type="time" name="hora_fin" value={formState.hora_fin} onChange={handleInputChange} className={inputStyles} />
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end mt-6">
                            ...
                            <button type="submit" className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 font-semibold">Registrar Solicitud</button>
                        </div>
                    </form>
                </div>
            </Modal>
